...
const toolbarConfig = {}
const editorConfig = { 
  placeholder: '一次可上传多张，图片不能超过22张，图片不能大于6M，为提高图片质量可上传到flickr.com，然后获取地址，再在内容中选择网络图片添加' ,
  MENU_CONF: {
    uploadImage: {
      ...
      server: backurl + 'api/'+ localStorage.getItem('appname') +'/upload/?uid='+localStorage.getItem('user_id'), //这是后台上传文件的处理地址
      ...
      customInsert(res:any,insertFn:any){
          ...
          console.log('返回编辑器的上传图片addr', addr)
      ...
      onBeforeUpload(file_field){
        ...
        if (file_field[keys[0]].size>10*1024*1024){
          // alert('文件太大，不能上传 ')
           // // editor.Alert('asdfas')
