...
<van-form @submit="onSubmit" style="margin-bottom:65px; ">
  ...
  <van-field v-model="responseData.product_type" name="title" label="产品类型"  readonly      />
    ...
    <van-field v-model="responseData.email" name="email" label="邮件地址" readonly />
    

    <van-col span="24" style="" class="text-content">
      
    </van-col>
    
    

    <van-field v-model="responseData.addr" @click="selectaddr" name="addr" label="收货人地址" :value="addr" />
    <van-col span="24" class="text-content">
     
    </van-col>
    <van-field v-model="responseData.notes" name="tongju" label="瞳距" readonly/>
    <van-col span="24" class="text-content">
    ...
    <div style="margin: 16px; ">
        <van-button round block type="primary" @click="router.push('/mygoodsorder');" >返回订单列表</van-button>
...
//   });
  ...
  const goods_orderid = route.query.order_detail;
   ...
   console.log('url44',url44)
    ...
    https.get((`/api/order/goodsorder/${goods_orderid}/`)).then(response => {
      // console.log('asdfas555111');
