    ...
    >
      ...
      <ul>
        ...
        <div class="order-buttons">
          ...
          <span :style="{  marginRight: '10px',    color: order.status === '已完成' ? 'green' : 'red'   }"> {{ order.status }}  </span>
        ...
        <span :style="{  marginLeft: '10px',   color: order.is_paid ? 'green' : 'red'  }"> {{  order.is_paid ? '已支付' : '未支付' }}  </span>
...
//  分组的算法
  ...
  // console.log('userid', user_id)
    https.get(('/api/myfood/yizhan_orders_ziqu/')).then(response => {
      // console.log('asdfas555111');
