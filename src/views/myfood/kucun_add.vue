<template>
  ...
  <div>
    ...
    <van-divider content-position="center">新增库存项</van-divider>
    <van-cell-group>
      <van-field v-model="newItem.name" label="商品名称" placeholder="请输入商品名称" />
      <van-field v-model="newItem.unit" label="单位" placeholder="如 个、份" />
      <van-field v-model.number="newItem.quantity" type="number" label="数量" placeholder="输入数量" />
      <div style="display: flex;justify-content: center; ">
        <van-button type="success" block @click="add_goods_kucun" style="margin-top: 10px;width:30%">
          新增库存项
        </van-button>
      </div>
    ...
    <div v-for="(item, index) in retdata4" :key="index" style="margin-bottom: 0px;margin-top: 20px;">
      <!-- 第一行：商品名和库存输入框 -->
      <div style="display: flex; align-items: center;">
        <span style="width: 30%; margin-left: 15px; font-size: small; margin-right: 8px;">
          {{ item.goodsname }}
        </span>
        ...
        <van-button size="small" type="primary" @click="editItem(item)" style="margin-right: 8px;"> 修改 </van-button>
        <van-button size="small" type="danger" @click="deleteItem(item, index)"> 删除 </van-button>
      </div>
...
const get_shopinfo = () => {
  ...
  https.get(('/api/myfood/detail2/' + user_id + '/')).then(response => {
    goodsid.value = response.id
    shopname.value = response.title
    yizhan_userid.value = response.yizhan_userid
    owner.value = response.owner
    console.log("asdfchenggongle")
    ...
    console.log('出错了2');
    showDialog({ message: '您的访问次数过多2，请一分钟后再访问' })
    console.error(error);
    // showToast('请重新登录');
  });
...
const add_goods_kucun = () => {
  ...
  const params = {
    // 首先是店的信息
    "shopname": shopname.value,   //商店名字
    "goodspage": 'myfood',  //商店页面分类
    "goodsid": goodsid.value,   //商店页面id
    'yizhanuserid': yizhan_userid.value, // 商品的数量数量值
    'goodsname': newItem.value.name, // 
    "kucun": newItem.value.quantity,  //价格
...
onMounted(() => {
  https.get('/api/myfood/list_kucun/').then(response => {
    console.log('fanhuishu ')

    // if (response.count===0){
    if (response.results.length === 0) {
      get_shopinfo()
      // showDialog({ message: "您无库存数据" })
