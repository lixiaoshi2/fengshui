...
<template>
  ...
  <van-nav-bar title="邻家网 - 送餐商品" left-text="返回" @click="router.go(-1);" left-arrow>
    <template #right>
  ...
  <div style="margin-bottom: 70px;" class="order-container " ref="containerRef">
    <!-- <h2>订单列表</h2> -->
    <div v-for="order in groupedOrders" :key="order.ordersid" class="order-item">
      <h4>订单号: {{ order.ordersid }}</h4>
      <p style="height: 13px; padding-bottom:0px ; margin-bottom: 0px;">总金额: {{ order.totalAmount.toFixed(2) }} 元 ,
        其中运费：{{ order.totalYunfei.toFixed(2) }} ，总数量: {{ order.totalQuantity }}</p>

      <p><span style="">地址：</span> {{ order.shouhuoaddr }}</p>
      <p><span style="">电话：</span> {{ order.shouhuophone }}</p>
      <p><span style="">时间：</span> {{ order.create_time }}</p>
      ...
      <ul>
        ...
        <div class="order-buttons">
          ...
          <span :style="{ marginRight: '10px', color: order.status === '已完成' ? 'green' : 'red' }"> {{ order.status
            }} </span>
          ...
          <button 
            v-if="!order.is_paid && order.status === '待处理'" 
            class="order-btn detail-btn" 
            @click="delorder(order.ordersid,order.yizhanuserid)"
          >
            删除订单
          </button>
          <!-- <button class="order-btn delete-btn">已支付</button> -->
          <span :style="{ marginLeft: '10px', color: order.is_paid ? 'green' : 'red' }"> {{ order.is_paid ? '已支付' :
            '未支付' }} {{ order.yizhanuserid }}</span>
  ...
  <!-- 备注对话框 -->
  <van-dialog v-model:show="dialogVisible" show-cancel-button title="添加备注" confirm-button-text="保存"
    cancel-button-text="取消" @confirm="saveBeizhu" @cancel="handleDialogClose">
    <van-field v-model="currentOrderBeizhu" placeholder="请输入备注内容，字数不要超过15个字" clearable />
  </van-dialog>
...
console.log('确认下单')
https.put('/api/myfood/myfoodorder_update_beizhu2/', values2).then(response => {
...
//  分组的算法
  ...
  const groupedOrders = computed(() => {
    const orderMap = listdata2.value.reduce((acc, item) => {
      ...
      const yizhanuserid = item.yizhanuserid
  ...
  // console.log('userid', user_id)
    https.get(('/api/myfood/myfoodlist_guanli/')).then(response => {
      // console.log('asdfas555111');
    ...
    // 删除订单
    const delorder = (id,yizhanuserid) =>{
      console.log('order',id, 'yizhanuser',yizhanuserid)
      https.delete(`/api/myfood/myfoodorder_del/${id}/`,{"yizhanuserid":yizhanuserid}).then(response => {
      // console.log('asdfas555111');
      console.log('fanhuizhi', response);
...
nav {
            ...
            width: 100%; /* 占满父容器宽度 */
            background-color: #f4f4f4; /* 背景颜色（可选） */
        }

        nav a {
            flex: 1; /* 每个 <a> 标签占相等的宽度 */
