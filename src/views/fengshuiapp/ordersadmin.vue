    ...
    <div v-for="order in orders" :key="order.id" class="bg-white rounded shadow p-4 mb-4 border-b-4 border-gray-200">
      ...
      <div   class="border-t pt-2">
        <div v-for="item in order.items" :key="item.id" class="flex items-center justify-between mb-2">
          <div @click="goToDetail(order.id)" class="flex items-center gap-3">
...
const loadOrders = async () => {
  const res = await https.get(`/api/fengshui/orders_admin/?page=${page.value}`)
...
const goToDetail = (id) => {
  router.push(`/orderadmin_detail/${id}`)
}
