...
<van-form @submit="onSubmit" style="margin-bottom:65px; margin-top:30px;">
    ...
    <van-field v-model="owner" name="owner" label="驿站联系人" placeholder="选择驿站返回后再点击可显示" @update:modelValue="handleInputChange" 
...
<!-- 这部分是 -->
    ...
    <van-field v-model="email" name="email" label="驿站电邮" placeholder="请输入备用电话电话" :value="email"
    :rules="[{ required: false, message: '请输入备用电话电话' }]" />

    <van-field v-model="weixin" name="weixin" label="驿站微信" placeholder="请输入微信" :value="weixin"
    :rules="[{ required: false, message: '请输入微信' }]" />

    
    <van-field v-model="yizhan_userid" name="addr" label="驿站user_id" placeholder="经营者用户id" readonly />
...
// 选择驿站
const choose_yizhan = () =>{
  // title.value = localStorage.setItem("yizhan_title",title)
  console.log('title111',title.value)
  ...
  if(owner.value!=null && owner.value!=''){
    ...
    localStorage.setItem("yizhan_name",'yizhan_title')
  }
  else{
...
onMounted(() => {
  ...
  owner.value = localStorage.getItem("username3")
  phone.value = localStorage.getItem("userphone3")
  addr.value = localStorage.getItem("fulladdr3")
  yizhan_userid.value = localStorage.getItem("user_id")
