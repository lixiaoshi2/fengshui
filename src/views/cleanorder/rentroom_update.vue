...
<template>
  <van-nav-bar title="修改  租房栏目" left-text="返回" @click=" router.go(-1);" left-arrow>
<template #right>
...
<van-form @submit="onSubmit" style="margin-bottom:65px; margin-top:30px;">
    ...
    <van-field v-model="title" name="title" label="标题" placeholder="请输入疏通补漏标题" :value="title"
        :rules="[{ required: true, message: '请输入疏通补漏标题' }]" />
    ...
    <div style="padding-left:20px; background-color: white; font-size: small ; color: rgb(143, 137, 137);"> 请与google地图对比，正确不用修改，否则用google地图的经纬度 </div>
    
    <van-field v-model="jingweidu" name="latitude" label="经纬度" placeholder="请与google地图对比，如果不对输入google的经度" :value="addr"   />
    <!-- <van-field v-model="latitude" name="latitude" label="经度" placeholder="请与google地图对比，如果不对输入google的经度" :value="addr"   />
    
    <van-field v-model="longitude" name="longitude" label="纬度" placeholder="请与google地图对比，如果不对输入google的纬度" :value="addr"   /> -->
    
    <!-- <weizhi v-model:fieldValue="weizhizhi" ref="weizhiref" ></weizhi> -->

    <van-field v-model="city" name="city" label="城市" placeholder="请输入备用电话电话" :value="city"
...
const onSubmit = (values) => {
  ...
  const [latitude2, longitude2] = jingweidu.value.split(','); 
  ...
  https.put('/api/rentroom/update/'+id.value+'/',values2).then(response => {
...
localStorage.setItem("appname", 'rentroom')

id.value = route.params.id;

  https.get(('/api/rentroom/detail/'+id.value+'/')).then(response => {
  // https.get(('/api/hotel/hotel/2/')).then(response => {
    ...
    if (!isNaN(parseFloat(latitude.value))) { // 判断值是否为有效的数字类型
      ...
      jingweidu.value = latitude.value
    } 
    longitude.value = retdata2.value.longitude
    if (!isNaN(parseFloat(response.longitude))) { // 判断值是否为有效的数字类型
      longitude.value = parseFloat(longitude.value).toFixed(6); // 将数字转换成字符串并保留两位小数
      jingweidu.value = jingweidu.value + ',' + longitude.value
    } 
    console.log('经纬度',jingweidu.value)
