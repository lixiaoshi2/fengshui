...
<template>
  ...
  <van-nav-bar title="邻家网 - 开锁配匙" left-text="返回" @click="router.go(-1);" left-arrow>
     <template #right>
   ...
   <div style="border-bottom: 1px solid rgb(244, 235, 235); margin-bottom: 10px;">
     ...
     <div style="width: 100%;display: flex;flex-direction: row;justify-content: space-between;">
       <div style="width: 80%;flex:0 0 80%;">
         ...
         <van-field v-model="location2" label="指定位置" placeholder="地址附近的开锁配匙" @click="router.push('/search_page2/')" />
       </div>
     </div>
   ...
   <div class="full-screen-layout"   >
     ...
     <div class="info">
       <div class="info-item">
         ...
         <span style="font-weight: bold; font-size: 15px; color:black" > <a style="color: black;" :href="`/openkey_detail/${item.id}`" @click.prevent="goToDetails(item.id)">
               {{ item.title  }}
             </a> </span>
       </div>
       <div class="info-item">
...
const spinning = ref(false);  // 正在加载的标志
 ...
 const change_unit = (distance) => {
   ...
   onBeforeRouteLeave((to, from, next) => {
     ...
     // if (to.path == '/openkey_detail') {
     // console.log('lixiaoshi2',to.path)
     if (to.path.indexOf('openkey_detail')) {
 ...
 onActivated(() => {
   ...
   if (push == 'true') {
     ...
     // alert("enter into openkey_list page,it's ispush is ")
     // 重置isPush的状态
     // route.meta.isPush = true;
     // router.go(1);
     console.log("ads")
 ...
 const loaddata = () => {
   ...
   var url = '/api/openkey/list2/?city=' + search_city
 ...
 const loaddata2 = () => {
   ...
   if (listNextUrl !== "null") {     //如果
     ...
     // 下面的代码是根据测试还是生产环境来提供listNextUrl，这步骤提供http://192.168.1.127:8000/api/openkey/list2/?city=Toronto&page=2
...
//  这是通过刷新页面后首次加载按照经纬度查询的首页
     ...
     navigator.geolocation.getCurrentPosition(
       (position) => {
        ...
        //  console.log('position', position.coords.latitude)
         ...
         url2.value = '/api/openkey/list2/?city=' + search_city + '&latitude=' + latitude.value + '&longitude=' + longitude.value
       ...
       (error) => {
        //  alert('出错了')
         url2.value = '/api/openkey/list2/?city=' + search_city + '&latitude=' + '43.811950' + '&longitude=' + '-79.3548376';
         switch (error.code) {
 ...
 useHead({
   title: `加拿大多伦多 找到您附近最近最好最便宜开锁配匙`,
   meta: [
     {
       name: 'keyword',
       content: `加拿大,多伦多,, 最好最便宜, 高质量,高品质,高性价比开锁配匙`
     },
     {
       name: 'description',
       content: `加拿大,多伦多,, 找到您身边最近最好最便宜,高性价比开锁配匙`
     }
   ]
 });
