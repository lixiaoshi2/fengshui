...
<template>
  <van-nav-bar title="修改 -开锁服务" left-text="返回" @click=" router.go(-1);" left-arrow>
<template #right>
...
const onSubmit = (values) => {
  ...
  https.put('/api/openkey/update/'+id.value+'/',values2).then(response => {
...
localStorage.setItem("appname", 'openkey')

id.value = route.params.id;

  https.get(('/api/openkey/detail/'+id.value+'/')).then(response => {
