...
<template>
  <van-nav-bar title="新闻" left-text="返回" @click=" this.$router.go(-1);" left-arrow>
<template #right>
...
<van-form @submit="onSubmit" style="margin-bottom:65px; margin-top:30px;">
    ...
    <van-field v-model="title" name="title" label="标题" placeholder="请输入新闻标题" :value="title"
...
const onSubmit = (values) => {
  ...
  https.post('/api/news/add/',values2).then(response => {
